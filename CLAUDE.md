...
- **HN login detection via response body**: HN returns "Bad login." in the HTML body on failure. Follow redirects (default axios), check body for "Bad login" — most robust across all Node.js versions, HTTP versions, and environments. Previous approaches (`response.request.path`, `maxRedirects: 0` + Location header) were fragile.
